...
DOCUMENTATION = """
    ...
    description:
        - This connection plugin does nothing
    author:
    version_added:
    notes:
...
class Connection(ConnectionBase):
    ...
    def _connect(self):
        ...
        if not self._connected:
            display.vvv(
                "ESTABLISH NOTHING CONNECTION FOR USER: {0}".format(
    ...
    def exec_command(self, cmd, in_data=None, sudoable=True):
        ...
        display.vvv("NOTHING: EXEC {0}".format(cmd))
    ...
    def run_ansiballz(self, cmd):
        ...
        display.vvv("create_temp_dir")
        return json.dumps(dict(changed=False, stdout='hello', stderr='', rc=0)).encode()
